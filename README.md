...
В проекте реализована система ролей пользователей с различными уровнями доступа:

### 1. Администратор
- **Полный доступ ко всему**
- Может просматривать, редактировать, удалять любые записи
- Может создавать новые записи
- Имеет доступ к логам
- Может редактировать фильтры
- Может обновлять методические данные
...
- **Имеет права** обновлять методические данные при помощи сбора обновления
- **Не может** управлять пользователями
- **Не имеет** доступа к административной панели

### 3. Пользователь
- **Может создавать и просматривать любые записи**
- **Не может** редактировать или удалять чужие записи других пользователей
- **Не может** просматривать логи
- **Не может** редактировать фильтры
- **Не может** обновлять методические данные
- **Не может** управлять пользователями
- **Не имеет** доступа к административной панели

### Структура файлов системы ролей

- `roles.js` - Определение ролей и их прав
- `userPermissions.js` - Пример использования и функции для проверки прав
